...
import {
  ...
  Zap, User, Info, ChevronDown, ChevronUp,
} from 'lucide-react';
...
const EMOTION_VOICES = [
  { id: 'tara',   label: 'Tara',   display: 'Sophia',   gender: 'F' },
  { id: 'leah',   label: 'Leah',   display: 'Aurora',   gender: 'F' },
  { id: 'jess',   label: 'Jess',   display: 'Chloe',    gender: 'F' },
  { id: 'mia',    label: 'Mia',    display: 'Isabella', gender: 'F' },
  { id: 'zoe',    label: 'Zoe',    display: 'Scarlett', gender: 'F' },
  { id: 'rebeca', label: 'Rebeca', display: 'Natalie',  gender: 'F' },
  { id: 'lisa',   label: 'Lisa',   display: 'Vivian',   gender: 'F' },
  { id: 'leo',    label: 'Leo',    display: 'Ethan',    gender: 'M' },
  { id: 'dan',    label: 'Dan',    display: 'Marcus',   gender: 'M' },
  { id: 'zac',    label: 'Zac',    display: 'Caleb',    gender: 'M' },
  { id: 'bob',    label: 'Bob',    display: 'Adrian',   gender: 'M' },
];
...
export default function CreateVoiceover() {
  ...
  const validateForm = ({ requireVoice = true } = {}) => {
    ...
    if (emotionMode) {
      if (!emotionVoice) {
        toast.error('Please select an Emotion Voice before generating.');
        return false;
      }
  ...
  const generateVoiceover = async () => {
    ...
    const savePayload = emotionMode
      ? { ...formData, status: 'pending', api_type: 2, voice_name: emotionVoice, voice_id: '', voice_url: '' }
      : { ...formData, status: 'pending', api_type: 1 };
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto space-y-8 pb-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <GlassCard className="p-6" hover={false}>
            {/* Header with Emotion Voice Mode toggle */}
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">Choose a Voice</h3>
                  <p className="text-sm text-slate-400">
                    {emotionMode ? 'Emotion Voice Mode - expressive AI voices with emotion tags' : 'Browse and select from our voice library'}
                  </p>
                </div>
              </div>

              {/* Emotion Voice Mode toggle */}
              ...
              <div>
                ...
                <div className="mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-medium text-slate-400 uppercase tracking-wide">Female</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {EMOTION_VOICES.filter(v => v.gender === 'F').map(v => (
                      <button
                        key={v.id}
                        type="button"
                        onClick={() => setEmotionVoice(v.id)}
                        className={cn(
                          'px-4 py-2 rounded-xl border text-sm font-medium transition-all',
                          emotionVoice === v.id
                            ? 'border-violet-500 bg-violet-500/20 text-white shadow-lg shadow-violet-500/20'
                            : 'border-slate-700 bg-slate-800/50 text-slate-300 hover:border-violet-500/40 hover:text-white',
                        )}
                      >
                        {v.display}
                      </button>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-medium text-slate-400 uppercase tracking-wide">Male</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {EMOTION_VOICES.filter(v => v.gender === 'M').map(v => (
                      <button
                        key={v.id}
                        type="button"
                        onClick={() => setEmotionVoice(v.id)}
                ...
                {emotionVoice && (
                  <div className="mt-4 flex items-center gap-2 text-sm text-violet-300">
                    ...
                    Selected: <span className="font-semibold">{EMOTION_VOICES.find(v => v.id === emotionVoice)?.display}</span>
                  </div>
                )}
              </div>
            ) : (
