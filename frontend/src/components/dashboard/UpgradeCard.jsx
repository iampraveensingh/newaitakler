...
export default function UpgradeCard({ compact = false, currentPlan, addons, isAgencySubUser = false }) {
  ...
  // addons from API is an object e.g. { AGENCY: true, unlimited: true }
  ...
  // Determine upgrade type: Agency (PRO plan without Agency addon) or PRO (FE/XTREME plan)
  const upgradeType        = hasPro ? 'agency' : 'pro';
  const benefits           = upgradeType === 'pro' ? proBenefits : agencyBenefits;
  const upgradeTitle       = upgradeType === 'pro' ? 'Upgrade to PRO' : 'Upgrade to Agency';
  const upgradeDescription = upgradeType === 'pro'
    ? 'Unlock the full power of VoiceAI'
    : 'Scale your voice business with team features';
  const badgeText          = upgradeType === 'pro' ? 'PRO' : 'AGENCY';
  const UpgradeIcon        = upgradeType === 'pro' ? Crown : Users;
  const iconGradient       = upgradeType === 'pro' ? 'from-amber-400 to-orange-500' : 'from-blue-400 to-indigo-500';
  const iconShadow         = upgradeType === 'pro' ? 'shadow-amber-500/30' : 'shadow-blue-500/30';
  const badgeGradient      = upgradeType === 'pro' ? 'from-amber-500 to-orange-500' : 'from-blue-500 to-indigo-500';
  const btnGradient        = upgradeType === 'pro'
  ...
  const cardGradient       = upgradeType === 'pro'
    ? 'from-violet-900/40 via-purple-900/30 to-pink-900/20 border-violet-500/30'
    : 'from-blue-900/40 via-indigo-900/30 to-violet-900/20 border-blue-500/30';
    ...
    <GlassCard className={`p-6 bg-gradient-to-br ${cardGradient} relative overflow-hidden`} hover={false}>
      ...
      <div className="relative">
        <div className="flex items-start gap-4 mb-5">
          ...
          >
            <UpgradeIcon className="w-7 h-7 text-white" />
          </motion.div>
          <div>
